...
sql <- paste(
  ...
  "WHERE DISCHARGE_DATE >= To_date('2023-08-01', 'YYYY-MM-DD')"
)
...
simple_2_query <- tbl(smra_conn, in_schema("ANALYSIS", "SMR01_PI")) |>
  select(LINK_NO, ADMISSION_DATE, DISCHARGE_DATE) |>
  filter(DISCHARGE_DATE >= To_date("2023-08-01", "YYYY-MM-DD"))
